...
Here you will find the class `GainPro` used during the process of imputation of missing values.
The `GainPro` class is responsible for handling the architecture of the Generative Adversarial Imputation GainPro (GAIN) 
...
Attributes
-----------
- `hypers` : Instance of the `Hypers` class containing hyperparameters used in the model training.
...
- __init__()
    ...
    Initializes the `GainPro` class by processing the hyperparameters used in the model training.
