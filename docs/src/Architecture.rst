...
- **GainPro**: Trains the model using the attributes from the `Data` class.
- **Hypers**: Stores hyperparameters and passes them to the network.
...
2. The `Data` module loads the dataset, which is used by the `GainPro`.
3. The `GainPro` requires hyperparameters from the `Hypers` class.
