...
class DatasetBuilder:
    def __init__(
        ...
        if not cfg.dataset_path.exists():
            raise FileNotFoundError(errno.ENOENT, os.strerror(errno.ENOENT), cfg.dataset_path.name)
    ...
    def _resolve_normalizer(
        ...
        if value != "auto":
            return value
        return MODEL_NORMALIZER_DEFAULTS.get(model_name.lower()).get("normalizer", "none")
